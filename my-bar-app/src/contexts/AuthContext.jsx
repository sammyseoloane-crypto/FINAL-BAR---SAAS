...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUserProfile = async (userId) => {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, email, role, tenant_id, tenants!tenant_id(name)')
        .eq('id', userId)
        .single();
  ...
  const signUp = async (email, password, role = 'customer', tenantId = null, tenantName = null) => {
    try {
      ...
      if (authData.user) {
        ...
        await logSignupVerificationEmail(email, authData.user.id, finalTenantId, role);
      }
  ...
  const resetPassword = async (email) => {
    try {
      ...
      await logPasswordResetEmail(email, userData?.id || null, userData?.tenant_id || null);

      ...
      const staffStats = staffData.map((staff) => ({
        ...
        completionRate:
          staff.tasks.length > 0
            ? Math.round(
              (staff.tasks.filter((t) => t.status === 'completed').length / staff.tasks.length) *
                  100,
            )
            : 0,
      }));
              ...
              <tbody>
                {staffStats.map((staff, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <span className="status-badge status-pending">{staff.pending}</span>
                    </td>
                    <td>
                      <span className="status-badge" style={{ background: '#d4af37' }}>
                        {staff.in_progress}
                      </span>
                    </td>
                    <td>
                      <span className="status-badge status-completed">{staff.completed}</span>
                    </td>
                    <td>

...
const MyPurchasesPage = () => {
  ...
  const getDisplayName = (transaction) => {
    ...
    return `Purchase - R${parseFloat(transaction.amount).toFixed(2)}`;
  };
          ...
          <div style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden' }}>
            {transactions.map((transaction, index) => (
              ...
              >
                ...
                {transaction.qr_codes &&
                  transaction.qr_codes.length > 0 &&
                  !isExpired(transaction.qr_codes[0]) && (
                  <div
                    style={{
                      background: '#f7fafc',
                      padding: '12px',
                ...
                {transaction.status === 'pending' && (
                  ...
                  >
                    ⏳ Waiting for staff to confirm payment. QR code will be generated after
                    confirmation.
                  </div>
                )}
              </div>
            ...
            >
              ...
              {selectedTransaction.qr_codes && selectedTransaction.qr_codes.length > 0 && (
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
                  <QRCodeGenerator value={selectedTransaction.qr_codes[0].code} size={280} />
                </div>
              )}

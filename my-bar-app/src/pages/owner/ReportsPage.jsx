...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (!userProfile?.tenant_id || !isLive) {
      return;
    }
  ...
  const productTypeData = () => {
    ...
    productAnalytics.forEach((p) => {
      if (!typeRevenue[p.type]) {
        typeRevenue[p.type] = 0;
      }
          ...
          <div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                />
                <span style={{ color: '#666' }}>{isLive ? 'Live Updates' : 'Paused'}</span>
              </div>
              ...
              <div className={`stat-card ${updateAnimation ? 'updating' : ''}`}>
                ...
                {stats.revenueGrowth !== 0 && (
                  <div
                    style={{
                      fontSize: '12px',
                      color: stats.revenueGrowth > 0 ? '#28a745' : '#dc3545',
                      marginTop: '5px',
                    }}
                  >
                    {stats.revenueGrowth > 0 ? '↑' : '↓'} {Math.abs(stats.revenueGrowth)}% vs
                    previous period
                  </div>
                )}
              </div>

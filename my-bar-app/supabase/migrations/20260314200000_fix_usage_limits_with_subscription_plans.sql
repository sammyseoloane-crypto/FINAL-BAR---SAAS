-- Migration: Fix check_usage_limit to use subscription_plans table
-- Date: 2026-03-14
-- Description: Update check_usage_limit to properly join with subscription_plans
--              to get limits based on the tenant's subscription tier

-- Drop the old function
DROP FUNCTION IF EXISTS check_usage_limit(UUID, TEXT);

-- Recreate function with correct joins to subscription_plans
...
-- Add helpful comment
COMMENT ON FUNCTION check_usage_limit IS 'Check current usage vs limits for staff, locations, products, or events based on subscription plan';
